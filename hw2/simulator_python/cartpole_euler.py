...
class CartPoleViewer:
    def __init__(self, sim: CartPoleSim, width: int = 1000, height: int = 500):
        self.sim = sim
        self.width = width
        self.height = height
        ...
        pygame.init()
        pygame.display.set_caption("CartPole Keyboard Control (Euler)")
    ...
    def draw(self, action: float, reward: float, done: bool):
        ...
        lines = [
            "Controls: LEFT / RIGHT arrows, R to reset, close window to quit",
            "Integrator: Euler",
...
def keyboard_controller(force_limit: float) -> float:
    ...
    try:
        while running:
            ...
            if sim.is_terminated():
                ...
                action = keyboard_controller(sim.cfg.force_limit)
